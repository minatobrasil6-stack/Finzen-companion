...
# ============================================================
# FINZEN — app.py
# Versión consolidada con Panel de Administrador:
...
def sign_in(email, password):
    if email == CORREO_ADMIN:
        # Acceso de administrador directo para pruebas demo o bypass si se desea
        pass
    try:
        ...
        # Fallback para permitir entrada al admin si Supabase falla o credenciales locales
        if email == CORREO_ADMIN:
            class DummyUser:
                def __init__(self, mail):
                    self.email = mail
            return DummyUser(email), None
        return None, str(e)
...
""",
    ...
    if df_tx.empty:
        st.info(
            ...
            "Ve al menú superior para agregar el primero."
        )
    else:
        df_tx = df_tx.copy()
...
with tab6:
    ...
    if not es_pro:
        ...
        st.caption(
            "💡 En la pestaña Registrar puedes marcar un gasto como "
...
La conversión COP/USD использует una tasa de referencia consultada
...
Si tienes dudas sobre tus datos, puedes contactarnos en: **minatobrasil6@gmail.com**
"""
        )
...
# ============================================================
# PANEL DE ADMINISTRADOR (EXCLUSIVO PARA minatobrasil6@gmail.com)
# ============================================================
if es_administrador():
    with tab_admin:
        st.subheader("🛡️ Panel de Control de Administrador")
        st.markdown(f"Bienvenido, administrador **{CORREO_ADMIN}**. Desde aquí puedes supervisar la plataforma.")

        col_a, col_b = st.columns(2)
        with col_a:
            st.metric("Estado de la Base de Datos", "Conectada" if db_connected else "Modo Demo")
        with col_b:
            st.metric("Tasa de Cambio Base", f"1 USD = {tasa_usd_cop():,.2f} COP")

        st.divider()
        st.markdown("#### 👥 Gestión de Usuarios y Accesos Globales")
        
        if db_connected:
            try:
                res_subs = supabase.table("subscriptions").select("*").execute()
                if res_subs.data:
                    df_subs = pd.DataFrame(res_subs.data)
                    st.dataframe(df_subs, use_container_width=True)
                else:
                    st.info("No hay registros de suscripciones en la tabla aún.")
            except Exception as e:
                st.warning(f"No se pudieron cargar las suscripciones de Supabase: {e}")
        else:
            st.info("La base de datos Supabase no está configurada o conectada en este entorno.")

        st.markdown("#### ⚙️ Acciones de Mantenimiento")
        if st.button("Limpiar caché de datos globales"):
            st.cache_data.clear()
